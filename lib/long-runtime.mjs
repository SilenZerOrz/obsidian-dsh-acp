...
export class LongRuntime {
  ...
  async prompt(params) {
    ...
    const updates = [];
    let lastText = ""; // captured for any caller that still wants the final text
    const bridge = new LLMStreamBridge({
      onUpdate: async (update) => {
        ...
        updates.push(update);
        // Track last text chunk so a legacy caller (or the dsh web route)
        // can render the final answer without re-reading session state.
        if (update.sessionUpdate === "agent_message_chunk" && update.content?.text) {
          lastText = update.content.text;
        }
      },
    });
