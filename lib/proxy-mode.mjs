    ...
    while (true) {
      ...
      for (const ev of events) {
        ...
        if (ev.event === "result") {
          ...
          // Fire-and-await onRequest, then POST the reply back so the server's
          // pending Promise resolves and long-runtime can continue.
...
async function handleServerRequest({
  ...
  try {
    ...
    result = await onRequest(method, params);
  } catch (e) {
