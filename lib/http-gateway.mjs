...
async function handleOfficialSessionPrompt(req, res, ctx, bridge, official, body, sessionId) {
  ...
  try {
    const result = await router.prompt(sessionId, {
      prompt: body.prompt,
      model: typeof body.model === "string" ? body.model : undefined,
      // Forward apply()'s session/update notifications to this SSE writer
      // VERBATIM as { method:"session/update", params } — the same envelope
      // the legacy path emits (verified in official-bridge-alignment.test).
      onUpdate: (params) => writer.emit("sessionUpdate", { method: "session/update", params }),
      signal: turnAbort.signal,
    });
    writer.emit("result", {
      stopReason: result.stopReason,
