...
async function handleSessionPrompt(req, res, ctx, bridge, official = null) {
  ...
  if (official?.enabled && !official?.servicesReady) {
    ...
    const sig = (official.missingServices ?? []).join(",");
    if (!official._warnedSig || official._warnedSig !== sig) {
      official._warnedSig = sig;
      process.stderr.write(
        `[dsh-acp] WARNING: official bridge opted-in via env but cordis ctx is ` +
        `missing services [${sig}]. Falling back to legacy long-runtime. ` +
        `This usually means dsh web profile lacks @deepseek-ai/dsh-llm / dsh-session / ` +
        `dsh-agent packages, or dsh-llm typert codec is incomplete. See ` +
