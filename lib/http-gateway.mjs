...
import { probeOfficialBridge } from "./official-acp-bridge.mjs";
...
async function handleSessionPrompt(req, res, ctx, bridge, official = null) {
  ...
  // Deliberately NO auto-fallback to legacy when official is usable: if
  // apply() mount/prompt fails the client sees an error, never a
  // silently-legacy "success". Rollback is the explicit env switch + restart.
  //
  // BUT: when official is ENABLED via env but UNUSABLE (servicesReady=false),
  // we log a one-shot stderr warning AND silently fall back to legacy so the
  // client still gets real output. The warning tells the operator WHY official
  // was skipped — otherwise they'd see model responses and assume official was
  // working. See registerHttpGateway() comment for the dsh 0.1.6-alpha.x
  // root cause.
  if (official?.usable) {
    await handleOfficialSessionPrompt(req, res, ctx, bridge, official, body, sessionId);
    return;
  }
  if (official?.enabled && !official?.servicesReady) {
    // Idempotent one-shot per process: dedupe by missingServices signature.
    const missing = official.missingServices ?? [];
    const detail = missing.length > 0
      ? `missing services [${missing.join(",")}]`
      : `services not ready (probe: ${official.probe?.error ?? "unknown"})`;
    const sig = missing.join(",") || `<error:${official.probe?.error ?? "unknown"}>`;
    if (!official._warnedSig || official._warnedSig !== sig) {
      official._warnedSig = sig;
      process.stderr.write(
        `[dsh-acp] WARNING: official bridge opted-in via env but ${detail}. ` +
        `Falling back to legacy long-runtime. This usually means dsh web profile ` +
        `lacks @deepseek-ai/dsh-llm / dsh-session / dsh-agent packages, or ` +
        `dsh-llm typert codec is incomplete. See ` +
        `docs/实施计划/0.3.0-rc.1-known-gaps.md §八 for the E2E finding.\n`,
      );
    }
  }
 ...
 *           ensureLongRuntime())
 */
export async function registerHttpGateway(ctx, ws, options = {}) {
  ...
  // #0.3.0-rc.1 fallback (2026-09-22 E2E finding): when official bridge is
  // opted in via env but the dsh cordis ctx is missing services (llm /
  // sessionPersistence / agents / sessions — common under dsh 0.1.6-alpha.x
  // web profile), apply() returns no frames and the client sees a silent empty
  // SSE stream. We must NOT silently route prompts through that broken path.
  // Below we eagerly probe to learn `servicesReady`; handleSessionPrompt uses
  // `official.usable` (effective "enabled && servicesReady") to decide the
  // route. When the user enabled the env but services are missing we emit a
  // one-shot stderr warning AND fall back to the legacy long-runtime path so
  // the client sees real model output instead of an empty stream.
  let officialProbe = null;
  if (officialEnabled) {
    try {
      // NOTE: probe runs ONCE at registerHttpGateway time. If dsh ever supports
      // lazy service injection (services registered after this mount), the
      // cached servicesReady would stay false and official would never route.
      // Known limitation — do not auto-re-probe per request (expensive).
      officialProbe = await probeOfficialBridge(ctx);
    } catch (e) {
      officialProbe = { servicesReady: false, error: `probe threw: ${e?.message ?? String(e)}` };
    }
  }
  const official = {
    enabled: officialEnabled,
    servicesReady: officialProbe?.servicesReady ?? false,
    missingServices: officialProbe?.missingServices ?? [],
    // Probe result, attached so handleSessionPrompt's "enabled && services not
    // ready" fallback warning branch can read .error across the function-scope
    // boundary (the local `officialProbe` is NOT visible inside handleSessionPrompt).
    probe: officialProbe,
    // NOTE: getter uses `this` — destructuring `const { usable } = official`
    // would break the binding. No current callers destructure it; keep access
    // via `official.usable`.
    get usable() {
      return this.enabled && this.servicesReady;
    },
    async getRouter() {
