...
export class SseWriter {
   ...
   *
   * Why this exists (2026-09-18 P3.0 tool-call fix):
   *   long-runtime.prompt() → permission gate → acpClient.request("session/request_permission")
   *   → needs to reach the Obsidian Agent Client via the same dsh-acp.mjs
   *   proxy that emitted this writer. SSE is one-way, so the reply travels
   *   back as a separate POST to /acp/proxy/permission-response (handled by
   *   handlePermissionResponse()) which finds this writer by sessionId+correlationId.
   *
   * @param {string} method - JSON-RPC method name (e.g. "session/request_permission")
   * @param {object} params
   * @returns {Promise<any>}
   */
  emitRequest(method, params) {
    if (this.closed) {
    ...
    return new Promise((resolve, reject) => {
      this._pendingRequests.set(correlationId, { resolve, reject, method });
 ...
 * @returns {{ notify: function, request: function }}
 */
export function makePerPromptAcpClient(writer) {
  const client = {
    ...
    async request(method, params) {
      return writer.emitRequest(method, params);
    },
  };
  return client;
}
...
async function handleSessionPrompt(req, res, ctx, bridge) {
  ...
  rt._liveAcpClient = makePerPromptAcpClient(writer);
